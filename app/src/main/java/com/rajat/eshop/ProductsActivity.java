...
//    String message = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        products =(ArrayList<Product>) new Gson().fromJson(getIntent().getStringExtra("data"),
...
//                }.getType());
        ...
        productsGridAdapter=new ProductsGridAdapter(this,count, products);
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if(position==1){
                    if (pref.getString("type","").equals("Shopkeeper")) {
                        VolleyClick.findProductsClick(pref.getString("userId",""), ProductsActivity.this);
                    } else {
                        VolleyClick.findDiscountsClick(pref.getString("userId",""), ProductsActivity.this);
                    }
                }
                else if(position==2){
                    VolleyClick.findOrdersClick(pref.getString("userId",""), pref.getString("type",""), ProductsActivity.this);
                }
                else if(position==3){
                    if (pref.getString("type", "").equals("Shopkeeper")) {
                        Intent intent = new Intent(ProductsActivity.this, CreateDiscount.class);
                        ProductsActivity.this.startActivity(intent);
                    } else {
                        VolleyClick.getSubscriptionClick(pref.getString("deviceId", ""), ProductsActivity.this);
                    }
                }
                else if(position==4){
                    Intent intent = new Intent(ProductsActivity.this, NotificationView.class);
                    ProductsActivity.this.startActivity(intent);
                }
                else if(position==5){
                    if (pref.getString("type", "").equals("Shopkeeper")) {
                        VolleyClick.logoutClick(pref.getString("deviceId", ""), ProductsActivity.this);
                    }else{
                        VolleyClick.findOffersClick(ProductsActivity.this);
                    }
